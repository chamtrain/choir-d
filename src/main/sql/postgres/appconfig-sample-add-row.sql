-- Sample PostGres script to ADD a new app_config row to the database
-- To see all rows having a string as part of their name, use:  appconfig-query.sql
\echo
\echo '== This script adds a new row to appconfig, using paraters:  site, cType, cName, cValue, yourUserId'
\echo '==   Note: yourUserId is your numeric user_principal.user_principal_id, not your user name'
\echo '==    e.g.:  select * from user_principal where username='admin';'
\echo '==   As of 3.0, ctype is one of (besides the emailtemplate and randomset you should not use this for):'
\echo '==      configparam,  custom,  squaretable,  surveycontent,  builder'
\echo

-- First, define your values- if the value has spaces, 
\set cName treatmentset.show.fullstate
\set cValue Y
\set ctype configparam
\set site 1
\set yourUserId 1
