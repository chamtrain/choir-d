...
/**
 * Custom copy of the Wrapper for the Summernote WYSIWYG Editor
   * <p/>
  ...
  public class SummernoteBase extends Div implements HasAllSummernoteHandlers, HasEnabled {
    ...
    public String getOption(final String option) {
      ...
      return getElement().getAttribute(option);
    }

    public void setOption(final String optionName, final String optionValue) {
      if (isAttached()) {
        setOption(getElement(), optionName, optionValue);
      } else {
        getElement().setAttribute(optionName, optionValue);
      }
    }
    ...
    private native void initialize(Element e, SummernoteOptions options) /*-{
      ...
      options.styleTags =  ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
    ...
    /*
     $wnd.jQuery(e).summernote({
        ...
        toolbar: [
              ...
              ['para', ['ul', 'ol', 'li', 'para', 'hr']],
              ['control', ['undo', 'redo', 'codeview']]
        ]
      });
*/
    ...
    private native String getOption(Element e, String optionName)/*-{
    return $wnd.jQuery(e).summernote(optionName);
  }-*/;

    private native void setOption(Element e, String optionName, String optionValue) /*-{
      $wnd.jQuery(e).summernote(optionName, optionValue);
    }-*/;
