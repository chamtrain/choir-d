...
import edu.stanford.survey.server.promis.PromisPhysicalFunctionMobility;
import edu.stanford.survey.server.promis.PromisPhysicalFunctionUpperExtremity;
...
/**
 * Implementation of surveys for my personal testing purposes.
 */
public class SurveySystemStub extends SurveySystemBase {
  ...
  private static final String CHOICE_THANKS = "Thank you page (ButtonListPage)";
  ...
  private static final String CHOICE_CAT_PHYSICAL_FUNCTION2 = "Physical Function v1.2 CAT (PROMIS)";
  private static final String CHOICE_CAT_PHYSICAL_FUNCTIONSTAN = "Physical Function CAT (Stanford)";
  private static final String CHOICE_CAT_PHYSICAL_FUNCTION_UPPER_EXTREMITY = "Physical Function - Upper Extremity CAT (PROMIS)";
  private static final String CHOICE_CAT_PHYSICAL_FUNCTION_MOBILITY = "Physical Function - Mobility CAT (PROMIS)";
  //private static final String CHOICE_CAT_PAIN_INTENSITY = "Pain Interference CAT";
  private static final String CHOICE_CAT_PAIN_INTERFERENCE1 = "Pain Interference v1.1 CAT";
  private static final String CHOICE_CAT_DEPRESSION = "Depression CAT";
  private static final String CHOICE_CAT_ANXIETY = "Anxiety CAT";
  private static final String CHOICE_CAT_ANGER = "Anger CAT";
  private static final String CHOICE_CAT_FATIGUE = "Fatigue CAT";
  private static final String CHOICE_CAT_PAIN_BEHAVIOR = "Pain Behavior CAT";
  private static final String CHOICE_CAT_SLEEP_DISTURBANCE = "Sleep Disturbance CAT";
  private static final String CHOICE_CAT_SLEEP_RELATED_IMPAIRMENT = "Sleep-Related Impairment CAT";
  private static final String CHOICE_COLLAPSIBLE_CONSENT = "Research Consent";
  private static final String CHOICE_COLLAPSIBLE_CONSENT2 = "Research Information Sheet - Revised Mar 2015";
  private static final String CHOICE_TREATMENTS_OPT1="Treatments - vertical sub questions";
  private static final String CHOICE_TREATMENTS_OPT2="Treatments - horizontal sub questions";
  private static final String CHOICE_COLLAPSIBLE_CONTENT_FORMFIELD = "Collapsible Content Form Field";
  private static final String CHOICE_DATE_PICKER = "Date Picker";
  private static final String CHOICE_GRID = "Radio grid without ranking";
  private static final String CHOICE_RANK = "Radio grid with ranking";
  ...
  public Question nextQuestion(Answer answer, Survey survey) {
    ...
    if ("index".equals(questionId)) {
      RadiosetAnswer indexAnswer = AutoBeanCodex.decode(factory, RadiosetAnswer.class, answer.getAnswerJson()).as();

      if (CHOICE_RADIOSET_PAGE.equals(indexAnswer.getChoice())) {
        return applesQuestion();
      } else if (CHOICE_SLIDER_PAGE.equals(indexAnswer.getChoice())) {
        return sliderQuestion();
      } else if (CHOICE_NUMERIC_SCALE_PAGE.equals(indexAnswer.getChoice())) {
        return numericScaleQuestion(true);
      } else if (CHOICE_NUMERIC_SCALE_PAGE2.equals(indexAnswer.getChoice())) {
        return numericScaleQuestion(false);
      } else if (CHOICE_NUMERIC_SCALE_PAGE3.equals(indexAnswer.getChoice())) {
        return numericScaleFormQuestion();
      } else if (CHOICE_BODY_MAP_MALE.equals(indexAnswer.getChoice())) {
        return bodyMapQuestionMale();
      } else if (CHOICE_BODY_MAP_FEMALE.equals(indexAnswer.getChoice())) {
        return bodyMapQuestionFemale();
      } else if (CHOICE_LOGIN.equals(indexAnswer.getChoice())) {
        return textInputQuestion();
      } else if (CHOICE_THANKS.equals(indexAnswer.getChoice())) {
        return thankYou();
      } else if (CHOICE_NUMBER.equals(indexAnswer.getChoice())) {
        return number();
      } else if (CHOICE_NUMBERS.equals(indexAnswer.getChoice())) {
        return numbers();
      } else if (CHOICE_TEXT.equals(indexAnswer.getChoice())) {
        return text();
      } else if (CHOICE_TEXT_AREA.equals(indexAnswer.getChoice())) {
        return textArea();
      } else if (CHOICE_CHECKBOXES.equals(indexAnswer.getChoice())) {
        return checkboxes();
      } else if (CHOICE_CHECKBOXES_REQUIRED.equals(indexAnswer.getChoice())) {
        return checkboxesRequired();
      } else if (CHOICE_CHECKBOXES_AND_OR.equals(indexAnswer.getChoice())) {
        return checkboxesAndOr();
      } else if (CHOICE_CONDITIONAL.equals(indexAnswer.getChoice())) {
        return conditional();
      } else if (CHOICE_CONDITIONAL_3_LEVEL.equals(indexAnswer.getChoice())) {
        return conditional3Level();
      } else if (CHOICE_CAT_PHYSICAL_FUNCTION2.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisPhysicalFunction2.bank(), "physicalFunction", answers);
      } else if (CHOICE_CAT_PHYSICAL_FUNCTIONSTAN.equals(indexAnswer.getChoice())) {
        answers.clear();
        ignore.clear();
        return cat2(PromisPhysicalFunction2.bank(), "physicalFunction", answers, ignore);
      } else if (CHOICE_CAT_PHYSICAL_FUNCTION_UPPER_EXTREMITY.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisPhysicalFunctionUpperExtremity.bank(), "physicalFunctionUpperExtremity", answers);
      } else if (CHOICE_CAT_PHYSICAL_FUNCTION_MOBILITY.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisPhysicalFunctionMobility.bank(), "physicalFunctionMobility", answers);
      }else if (CHOICE_CAT_PAIN_INTERFERENCE1.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisPainInterference1.bank(), "painInterference", answers);
      } else if (CHOICE_CAT_DEPRESSION.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisDepression2.bank(), "depression", answers);
      } else if (CHOICE_CAT_ANXIETY.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisAnxiety2.bank(), "anxiety", answers);
      } else if (CHOICE_CAT_PAIN_BEHAVIOR.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisPainBehavior.bank(), "painBehavior", answers);
      } else if (CHOICE_CAT_ANGER.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisAnger.bank(), "anger", answers);
      } else if (CHOICE_CAT_FATIGUE.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisFatigue2.bank(), "fatigue", answers);
      } else if (CHOICE_CAT_SLEEP_DISTURBANCE.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisSleepDisturbance.bank(), "sleepDisturbance", answers);
      } else if (CHOICE_CAT_SLEEP_RELATED_IMPAIRMENT.equals(indexAnswer.getChoice())) {
        answers.clear();
        return cat(PromisSleepRelatedImpairment.bank(), "sleepRelatedImpairment", answers);
      } else if (CHOICE_COLLAPSIBLE_CONSENT.equals(indexAnswer.getChoice())) {
        return consentQuestion();
      } else if (CHOICE_COLLAPSIBLE_CONSENT2.equals(indexAnswer.getChoice())) {
        return consentQuestion2();
      } else if (CHOICE_TREATMENTS_OPT1.equals(indexAnswer.getChoice())) {
        return treatmentsQuestionVertical();
      } else if (CHOICE_TREATMENTS_OPT2.equals(indexAnswer.getChoice())) {
        return treatmentsQuestionHorizontal();
      } else if (CHOICE_COLLAPSIBLE_CONTENT_FORMFIELD.equals(indexAnswer.getChoice())) {
        return collapsibleContentFormFieldQuestion();
      } else if (CHOICE_DATE_PICKER.equals(indexAnswer.getChoice())) {
        return datePickerFormFieldQuestion();
      } else if (CHOICE_GRID.equals(indexAnswer.getChoice())) {
        return radioGridQuestion();
      } else if (CHOICE_RANK.equals(indexAnswer.getChoice())) {
        return rankedRadioGridQuestion();
      }
      throw new RuntimeException("Unknown selection on buttonList: " + indexAnswer.getChoice());
    } else if ("textInput".equals(questionId)) {
      TextInputAnswer textAnswer = AutoBeanCodex.decode(factory, TextInputAnswer.class, answer.getAnswerJson()).as();

      String pin = textAnswer.getChoice().get("PIN code");
      if (pin == null || !pin.equals("123")) {
        Question q = textInputQuestion();
        q.getDisplayStatus().setSessionStatus(SessionStatus.questionInvalid);
        q.getDisplayStatus().setServerValidationMessage("You should have typed \"123\" for the PIN code");
        return q;
      }

      return index();
    } else if (answer.providedBy("cat:physicalFunction")) {
      ItemBank bank = PromisPhysicalFunction2.bank();
      Response response = bank.response(questionId, answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "physicalFunction", answers);
    } else if (answer.providedBy("cat2:physicalFunction")) {
      ItemBank bank = PromisPhysicalFunction2.bank();
      ...
      return cat2(bank, "physicalFunction", answers, ignore);
    } else if (answer.providedBy("cat:physicalFunctionUpperExtremity")) {
      ItemBank bank = PromisPhysicalFunctionUpperExtremity.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "physicalFunctionUpperExtremity", answers);
    } else if (answer.providedBy("cat:physicalFunctionMobility")) {
      ItemBank bank = PromisPhysicalFunctionMobility.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "physicalFunctionMobility", answers);
    }else if (answer.providedBy("cat:painInterference")) {
      ItemBank bank = PromisPainInterference1.bank();
      Response response = bank.response(questionId, answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "painInterference", answers);
    } else if (answer.providedBy("cat:depression")) {
      ItemBank bank = PromisDepression2.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "depression", answers);
    } else if (answer.providedBy("cat:anxiety")) {
      ItemBank bank = PromisAnxiety2.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "anxiety", answers);
    } else if (answer.providedBy("cat:painBehavior")) {
      ItemBank bank = PromisPainBehavior.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "painBehavior", answers);
    } else if (answer.providedBy("cat:anger")) {
      ItemBank bank = PromisAnger.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "anger", answers);
    } else if (answer.providedBy("cat:fatigue")) {
      ItemBank bank = PromisFatigue2.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "fatigue", answers);
    } else if (answer.providedBy("cat:sleepDisturbance")) {
      ItemBank bank = PromisSleepDisturbance.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "sleepDisturbance", answers);
    } else if (answer.providedBy("cat:sleepRelatedImpairment")) {
      ItemBank bank = PromisSleepRelatedImpairment.bank();
      Response response = bank.response(answer.questionId(), answer.formFieldValue("response"));
      if (response != null) {
        answers.add(response);
      }
      return cat(bank, "sleepRelatedImpairment", answers);
    } else {
      return index();
    }
  }

  static double priorTheta = 0;
//  static List<Response> prior = new ArrayList<Response>();   // TODO use the stored answers once available
  static List<Response> answers = new ArrayList<>(); // TODO use the stored answers once available
  static List<Item> ignore = new ArrayList<>(); // TODO use the stored answers once available
//  static List<Item> priorIgnore = new ArrayList<Item>(); // TODO use the stored answers once available
  static double progress = 0;

  private Question cat(ItemBank bank, String bankName, List<Response> answers) {
    CatAlgorithmPromis cat = new CatAlgorithmPromis();
    cat.initialize(bank);
    Item item = cat.nextItem(answers);
    if (item == null) {
      StringBuilder buf = new StringBuilder();
      List<Response> priorAnswers = new ArrayList<>();
      for (Response response : answers) {
        priorAnswers.add(response);
        Score score = cat.score(priorAnswers);
        int step = priorAnswers.size();
        buf.append("Step ").append(step).append(": ").append(response.item().promptBrief()).append(": ")
            .append(response.text()).append("<br/>");
        buf.append("&nbsp;&nbsp;&nbsp;&nbsp;score: ").append(score).append("<br/>");
        progress = 100;
      }
      Question q = new Question("done:" + bankName);
      q.form(buf.toString());
      return q;
    }
    ...
    q.getDisplayStatus().setSurveyProviderId("cat:" + bankName);
    return q;
  }

  private Question cat2(ItemBank bank, String bankName, List<Response> answers, List<Item> ignore) {
    ...
    q.getDisplayStatus().setSurveyProviderId("cat2:" + bankName);
    return q;
  }
  ...
  private Question index() {
    ...
    Question q = new Question("index", QuestionType.buttonList);
    ...
    choices.add(CHOICE_CAT_PHYSICAL_FUNCTION2);
    choices.add(CHOICE_CAT_PHYSICAL_FUNCTIONSTAN);
    choices.add(CHOICE_CAT_PHYSICAL_FUNCTION_UPPER_EXTREMITY);
    choices.add(CHOICE_CAT_PHYSICAL_FUNCTION_MOBILITY);
    choices.add(CHOICE_CAT_PAIN_INTERFERENCE1);
    choices.add(CHOICE_CAT_DEPRESSION);
    choices.add(CHOICE_CAT_ANXIETY);
    choices.add(CHOICE_CAT_PAIN_BEHAVIOR);
    choices.add(CHOICE_CAT_ANGER);
    choices.add(CHOICE_CAT_FATIGUE);
    choices.add(CHOICE_CAT_SLEEP_DISTURBANCE);
    choices.add(CHOICE_CAT_SLEEP_RELATED_IMPAIRMENT);
    choices.add(CHOICE_COLLAPSIBLE_CONSENT);
    choices.add(CHOICE_COLLAPSIBLE_CONSENT2);
    choices.add(CHOICE_TREATMENTS_OPT1);
    choices.add(CHOICE_TREATMENTS_OPT2);
    choices.add(CHOICE_COLLAPSIBLE_CONTENT_FORMFIELD);
    choices.add(CHOICE_DATE_PICKER);
    choices.add(CHOICE_GRID);
    choices.add(CHOICE_RANK);
  ...
  private Question applesQuestion() {
    Question q = new Question("apples", QuestionType.radioset);
  ...
  private Question sliderQuestion() {
    Question q = new Question("slider", QuestionType.slider);
  ...
  private Question numericScaleQuestion(boolean addLabels) {
    Question q = new Question("numericScale", QuestionType.numericScale);
  ...
  private Question numericScaleFormQuestion() {
    Question q = new Question("numericScale", QuestionType.form);

    FormField withLabels = q.field("with", FieldType.numericScale, "How intense was your pain WITH labels?");
    withLabels.setMin("0");
    ...
    FormField withoutLabels = q.field("without", FieldType.numericScale, "How intense was your pain WITHOUT labels?");
    withoutLabels.setMin("0");
    withoutLabels.setMax("10");

    q.form("How do <em>you</em> like <b>bold</b>, <i>italic</i>,"
  ...
  private Question bodyMapQuestionMale() {
    Question q = new Question("maleBodyMap", QuestionType.bodyMap);
  ...
  private Question bodyMapQuestionFemale() {
    Question q = new Question("femaleBodyMap", QuestionType.bodyMap);
  ...
  private Question textInputQuestion() {
    Question q = new Question("textInput", QuestionType.textList);
  ...
  private Question thankYou() {
    Question q = new Question("thanks");

    // Make the video field
    FormField videoField = q.field("thankYou" , FieldType.videoLink);
    ...
    q.form("Thank you for completing this questionnaire.<p>For additional information please take a few moments to watch the following video",
        videoField);
    return q;
  }
  ...
  private Question number() {
    ...
    q.form("In the past 6 months, how many times did you visit a physician?",
        "Do not include visits while in the hospital or to a hospital emergency room.",
        q.required(q.range(q.field("physicianVisits", FieldType.number, "Number of visits"), "0", "200"))
    );

    return q;
  }

  private Question numbers() {
    Question q = new Question("numbers");

    q.form("How long have you had your pain problem?",
        q.field("painYears", FieldType.number, "Years"),
        q.field("painMonths", FieldType.number, "Months"),
        q.field("painDays", FieldType.number, "Days")
    );
  ...
  private Question text() {
    ...
    q.form("Who is your primary care physician?",
        q.required(q.field("physician", FieldType.text))
    );

    return q;
  }

  private Question textArea() {
    Question q = new Question("text");

    q.form("What are the specific question(s) that you or your doctor want answered today?",
        q.required(q.field("questions", FieldType.textArea))
    );

    return q;
  }

  private Question checkboxes() {
    Question q = new Question("checkboxes");

    q.form("Describe your current pain",
        q.field("currentPainDesc", FieldType.checkboxes,
            q.value("Throbbing"),
            q.value("Shooting"),
            q.value("Stabbing"),
            q.value("Sharp"),
  ...
  private Question checkboxesRequired() {
    Question q = new Question("checkboxes");

    q.form("Describe your current pain",
        q.required(q.field("currentPainDesc", FieldType.checkboxes,
            q.value("Throbbing"),
            q.value("Shooting"),
            q.value("Stabbing")
        ))
  ...
  private Question checkboxesAndOr() {
    Question q = new Question("checkboxesAndOr");

    q.form("Please describe the timing of your pain",
        q.field("painTiming", FieldType.checkboxes,
            q.value("Brief"),
            q.value("Constant"),
            q.value("Comes and goes"),
            q.value("Continuous"),
            q.value("Always there"),
            q.value("Appears and disappears"),
            q.value("Intermittent")
        ),
        q.field("painTimingOther", FieldType.textArea, "and/or")
    );

    return q;
  }

  private Question conditional() {
    Question q = new Question("checkboxesAndOr");
  ...
  private Question conditional3Level() {
    Question q = new Question("checkboxesAndOr");

    q.form("Are you receiving any kind of disability?",
        q.required(q.field("disability", FieldType.radios,
            q.value("No"),
            q.value("Yes",
                q.required(q.field("disabilityType", FieldType.radios, "What kind of disability?",
                    q.value("Worker's Compensation"),
                    q.value("Social Security Disability Insurance (SSDI)"),
                    q.value("Other",
                        q.required(q.field("disabilityTypeOther", FieldType.textArea, "What other kind?"))
                    )
                ))
            )
        ))
  ...
  private Question consentQuestion() {
    Question q = new Question("consentQ", QuestionType.collapsibleRadioset);
    ...
    question.setCollapsibleHeading("More Info");
  ...
  private Question consentQuestion2() {
    Question q = new Question("consentQ", QuestionType.collapsibleRadioset);

    AutoBean<CollapsibleRadiosetQuestion> bean = factory.collapsibleRadiosetQuestion();
    CollapsibleRadiosetQuestion question = bean.as();

    question.setTitle1("Would you like to sign up for our Research Database so that our research team may contact you to discuss research opportunities?");
    question.setCollapsibleHeading("More Info");

    String content = "<p><b>DESCRIPTION:</b> You are invited to participate in a research database for current and future studies regarding pain. "
                    +"Indicating \"Yes\" to the research database:"
                    + "<ul>"
                    + "   <li>Allows us to access information from your medical record (e.g. name, contact information, age, pain condition) to match you with research studies for which you may be eligible.</li>"
                    + "   <li>It does not obligate you to be a research participant, but allows us to contact you if we think you may be eligible for one of our research studies.</li>"
                    + "</ul></p>"
                    + "<p><b>TIME INVOLVEMENT:</b> Your participation will take approximately 5 minutes.</p>"
                    + "</p>"
                    + "<p><b>PAYMENTS:</b> You will not be paid to participate in this study.</p>"
                    + "</p>"
                    + "<p><b>PARTICIPANT'S RIGHTS:</b> If you have read this form and have decided to participate in this project, "
                    ...
                    + "otherwise entitled. The alternative is not to participate. You have the right to refuse to answer "
                    ...
                    + "<p><b>RISKS AND BENEFITS:</b>  There is a small risk that people outside our research lab "
                    + "could learn this information, but we do our best to keep all your information confidential. The "
                    + "benefits which may reasonably be expected to result from this study are that you "
                    + "may be found to be a candidate for one or more studies conducted at our lab. We cannot and do not "
                    + "guarantee or promise that you will receive any benefits from this study. Your "
                    + "decision whether or not to participate in this study will not affect your medical care. </p>"
                    + "<hr>"
                    + "<h4><b>Authorization to Use Your Health Information for Research Purposes</b></h4>"
                    + "<hr>"
                    + "<p>This form is intended to inform you about how your health information will be used or disclosed in the study. Your information will only be used in accordance with this authorization form and the informed consent form and as required or allowed by law.</p>"
                    + "<p><b>What is the purpose of this research study and how will my health information be utilized in the study?</b></p>"
                    + "<p>The purpose of this research study is to add you to our database of potential study volunteers and check your eligibility for current or future studies.</p>"
                    + "<p><b>If I sign, can I revoke it or withdraw from the research later?</b></p>"
                    + "<p>If you decide to participate, you are free to withdraw your authorization regarding the use and disclosure of your health information (and to discontinue any other participation in the study) at any time. After any revocation, your health information will no longer be used or disclosed in the database, except to the extent that the law allows us to continue using your information (e.g., necessary to maintain integrity of research). If you wish to revoke your authorization for the research use or disclosure of your health information in this study, you must write to: Dr. Sean Mackey, 1070 Arastradero Road, Suite 200, Palo Alto, CA, 94304</p>"
                    + "<p><b>What Personal Information Will Be Obtained, Used or Disclosed?</b></p>"
                    + "<p>Information from you medical record, including but not limited to, your name, contact information, age, pain condition, and treatments could be utilized in this study.</p>"
                    + "<p><b>Who May Use or Disclose the Information?</b></p>"
                    ...
                    + "<ul>"
                    + "   <li>The Protocol Director, Sean Mackey, MD, PhD</li>"
                    ...
                    + "</ul></p>"
                    + "<p><b>Who May Receive or Use the Information?</b></p>"
                    + "The parties listed in the preceding paragraph may disclose your health information to the following persons and organizations for their use in connection with this research database:</p>"
                    + "<ul>"
                    ...
                    + "<p><b>When will my authorization expire?</b></p>"
                    + "Your authorization for the use and/or disclosure of your health information will end on December 31, 2100 or when the research project ends, whichever is earlier.</p>"
                    + "<p><b>CONTACT INFORMATION:</b></p>"
                    + "<p>Questions, Concerns, or Complaints: If you have any questions, concerns or complaints about this research database, its procedures, risks and benefits, you should ask the Protocol Director, Dr. Sean Mackey at (650) 725-9636.</p>"
                    ...
                    + "<p>Please print a copy of this form for your records.</p>";

    question.setCollapsibleContent(content);
    ArrayList<String> choices = new ArrayList<>();
    choices.add("Yes");
    choices.add("No");
    choices.add("Ask me again later");
  ...
  private Question treatmentsQuestionHorizontal() {

    Question q = new Question("checkboxesAndOr", QuestionType.form);
    q.form("Which of these opioid pain medications have you taken?",
    q.field("1",FieldType.checkboxes, q.value("Stadol <span class=\"fontnormal\">(Butorphanol)</span>", getConditional(q,true))),
    q.field("2",FieldType.checkboxes, q.value("Codeine, Tylenol #3,#4",getConditional(q, true))),
    q.field("3",FieldType.checkboxes, q.value("Actiq <span class=\"fontnormal\">(Fentanyl lozenges)</span>",getConditional(q, true))),
    q.field( "4",FieldType.checkboxes, q.value("Duragesic <span class=\"fontnormal\">(Fentanyl patch)</span>",getConditional(q, true))),
    q.field( "5",FieldType.checkboxes, q.value("Vicodin, Lortab, Norco <span class=\"fontnormal\">(Hydrocodone)</span>",getConditional(q, true))),
    q.field("6",FieldType.checkboxes, q.value("Dilaudid <span class=\"fontnormal\">(Hydromorphone)</span>",getConditional(q, true))),
    q.field( "7",FieldType.checkboxes, q.value("Demerol <span class=\"fontnormal\">(Meperidine)</span>", getConditional(q,true))),
    q.field( "8",FieldType.checkboxes, q.value("Methadone", getConditional(q,true))),
    q.field( "9",FieldType.checkboxes, q.value("MS Contin, Avinza, Kadian <span class=\"fontnormal\">(Morphine)</span>", getConditional(q,true))),
    q.field("10",FieldType.checkboxes, q.value("Percocet, Roxycodone, Oxycontin <span class=\"fontnormal\">(Oxycodone)</span>", getConditional(q,true))),
    q.field( "11",FieldType.checkboxes, q.value("Opana <span class=\"fontnormal\">(Oxymorphone)</span>", getConditional(q,true))),
    q.field( "12",FieldType.checkboxes, q.value("Nucynta <span class=\"fontnormal\">(Tapentadol)</span>", getConditional(q,true))),
    q.field( "13",FieldType.checkboxes, q.value("Ultram <span class=\"fontnormal\">(Tramadol)</span>", getConditional(q,true))));
    return q;
  }

  private Question treatmentsQuestionVertical() {
    Question q = new Question("checkboxesAndOr", QuestionType.form);
    q.form("Which of these mood medications have you taken? ",
    q.field("2", FieldType.checkboxes, q.value("Elavil <span class=\"fontnormal\">(Amitriptyline)</span>",getConditional(q, false))),
    q.field( "3", FieldType.checkboxes, q.value("Abilify <span class=\"fontnormal\">(Aripiprazole)</span>", getConditional(q, false))),
    q.field(  "4", FieldType.checkboxes, q.value("Wellbutrin <span class=\"fontnormal\">(Buproprion)</span>", getConditional(q, false))),
    q.field(  "5",FieldType.checkboxes, q.value( "Celexa <span class=\"fontnormal\">(Citalopram)</span>", getConditional(q, false))),
    q.field(  "6", FieldType.checkboxes, q.value("Desipramine ", getConditional(q, false))),
    q.field(  "7", FieldType.checkboxes, q.value("Pristiq <span class=\"fontnormal\">(Desvenlafaxine)</span>", getConditional(q, false))),
    q.field(  "8", FieldType.checkboxes, q.value("Cymbalta <span class=\"fontnormal\">(Duloxetine)", getConditional(q, false))),
    q.field(  "9", FieldType.checkboxes, q.value("Lexapro <span class=\"fontnormal\">(Escitalopram)", getConditional(q, false))),
    q.field(  "10",FieldType.checkboxes, q.value("Prozac <span class=\"fontnormal\">(Fluoxetine)</span>", getConditional(q, false))),
    q.field(  "11", FieldType.checkboxes, q.value("Savella <span class=\"fontnormal\">(Milnacipran)</span>", getConditional(q, false))),
    q.field(  "12",FieldType.checkboxes, q.value("Remoron <span class=\"fontnormal\">(Mitrazepine)</span>", getConditional(q, false))),
    q.field(  "13",FieldType.checkboxes, q.value("Serzone <span class=\"fontnormal\">(Nefazadone)</span>", getConditional(q, false))),
    q.field(  "14", FieldType.checkboxes, q.value("Pamelor <span class=\"fontnormal\">(Nortriptyline)</span>", getConditional(q, false))),
    q.field(  "15", FieldType.checkboxes, q.value("Paxil <span class=\"fontnormal\">(Paroxetine)", getConditional(q, false))),
    q.field(  "16", FieldType.checkboxes, q.value("Zoloft <span class=\"fontnormal\">(Sertraline)</span>", getConditional(q, false))),
    q.field( "17",FieldType.checkboxes, q.value( "Deseryl <span class=\"fontnormal\">(Trazadone)</span>", getConditional(q, false))),
    q.field( "18", FieldType.checkboxes, q.value("Effexor <span class=\"fontnormal\">(Venlafaxine)</span>", getConditional(q, false))));
    return q;
  }

  private FormField getConditional(Question q, boolean horizontal) {
    FormField conditionalField = q.field("meds-vertical-" + horizontal, FieldType.checkboxes,
        q.value("Still taking"),
              q.value("Effective"),
              q.value("Experienced side effects")
              );
      if (horizontal) {
        HashMap <String, String> attributes = new HashMap<>();
        attributes.put("Align","horizontal");
        conditionalField.setAttributes(attributes);
      }
  ...
  public Question collapsibleContentFormFieldQuestion() {
    Question q = new Question("migrain1", QuestionType.form);

    FormField fieldQ1 = factory.field().as();
    fieldQ1.setType(FieldType.collapsibleContentField);
    fieldQ1.setLabel("Question clarification");
    fieldQ1.setFieldId("1:0:CHRONMIGR1");
    Map<String, String> attrs = new HashMap<>();
    attrs.put("collapsibleContent", "if you do not remember the exact number of headache days, please give the best answer you can. If a headache lasted more than 1 day, count each day.</p>");
    attrs.put("icon", "INFO"); // optional, if not included the plus sign is used by default
    fieldQ1.setAttributes(attrs);

    FormField fieldQ2 = factory.field().as();
    fieldQ2.setType(FieldType.number);
    fieldQ2.setFieldId("1:1:CHRONMIGR1");
    fieldQ2.setLabel("Number of days");

    q.form("In the last 3 months (past 90 days), on how many days did you have a headache of any type?", fieldQ2, fieldQ1);

    return q;
  }

  public Question datePickerFormFieldQuestion() {
    Question q = new Question("datepicker");

    FormField field1 = factory.field().as();
    field1.setType(FieldType.datePicker);
    field1.setLabel("With popup picker on focus ");
    Map<String, String> attrs1 = new HashMap<>();
    attrs1.put("useFocus", "true");
    field1.setAttributes(attrs1);

    FormField field2 = factory.field().as();
    field2.setType(FieldType.datePicker);
    field2.setLabel("With inline picker and allow keying in date ");
    Map<String, String> attrs2 = new HashMap<>();
    attrs2.put("inlineBlind", "true");
    attrs2.put("lockInput", "false"); // optional, if not included the plus sign is used by default
    attrs2.put("styleName", ".dtpicker-right");
    field2.setAttributes(attrs2);


    q.form("Choose date from date picker",
        field1, field2);

    return q;
  }

  public Question radioGridQuestion() {
    Question q = new Question("radiogrid");
    q.form("With 5 being the Most true and 1 being the Least, <p>please rate each statement below <p>",
        q.field("1:1", FieldType.radioSetGrid, q.value("y-axis:1", "Most people I meet are friendly."),
            q.value("y-axis:2",  "Most dogs I meet are friendly."),
            q.value( "y-axis:3",  "Most cats I meet are friendly."),
            q.value("x-axis:1", "5"), q.value("x-axis::2", "4"), q.value("x-axis:3", "3"),
            q.value("x-axis:4", "2"), q.value("x-axis:5", "1")
       )
    );
    return q;
  }

  public Question rankedRadioGridQuestion() {
    Question q = new Question("radiogrid");
    FormField field = factory.field().as();
    field.setFieldId("1:1");
    field.setType(FieldType.radioSetGrid);
    ArrayList<FormFieldValue> values = new ArrayList<>();
    values.add(getGridValue("y-axis:WORLD", "... be in my own world."));
    values.add(getGridValue("y-axis:THINGS",  "... have a strong desire for things."));
    values.add(getGridValue( "y-axis:THINKING",  "... be critical, but clear thinking."));
    values.add(getGridValue("x-axis:1", "Most like me"));
    values.add(getGridValue("x-axis:2", "Not as much"));
    values.add(getGridValue("x-axis:3", "Least like me"));
    field.setValues(values);
    field.setAttributes(getRankedAttribute( ));
    q.form("Please rank the following in the order that is most consistent with how you generally behave "
            + "(not how you think you should behave, or how you might behave in a very specific situation)"
            +"<p>Generally, I tend to...", field);
    return q;
  }

  private FormFieldValue getGridValue( String id, String label ) {
    FormFieldValue formFieldValue = factory.value().as();
    formFieldValue.setId(id);
    formFieldValue.setLabel(label);
    return formFieldValue;
  }

  private HashMap<String, String> getRankedAttribute( ) {
    HashMap<String, String> attributes = new HashMap<>();
    attributes.put("Ranking", "true");
    return attributes;
  }
}
