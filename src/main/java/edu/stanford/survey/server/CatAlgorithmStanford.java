 ...
 * version is intended to provide fewer questions and more features than
 * {@link CatAlgorithmPromis}:
 * <p><ul>
 *   <li>Improved stopping conditions (stop when both score and SE are stable)</li>
 *   <li>Question skipping</li>
 * </ul></p>
 *
 * This implementation does not honor the priorTheta. That parameter is ignored
 * if provided.
 */
public class CatAlgorithmStanford implements CatAlgorithm2 {
  private static final Logger log = Logger.getLogger(CatAlgorithmStanford.class);
  private final double scoreStabilityThreshold;
  private final double errorStabilityThreshold;
  ...
  public CatAlgorithmStanford() {
    this(0.3d, 0.1d);
  }

  public CatAlgorithmStanford(double scoreStabilityThreshold, double errorStabilityThreshold) {
    this.scoreStabilityThreshold = scoreStabilityThreshold;
    this.errorStabilityThreshold = errorStabilityThreshold;
  }

  public double getScoreStabilityThreshold() {
    return scoreStabilityThreshold;
  }

  public double getErrorStabilityThreshold() {
    return errorStabilityThreshold;
  }
  ...
  public Item nextItem(List<Response> currentAssessmentResponses, List<Item> currentIgnored) {
    if (currentAssessmentResponses != null && currentAssessmentResponses.size() >= itemBank.minItems()) {
      ...
      if (currentAssessmentResponses.size() > 1) {
        ...
        if (Math.abs(currentScore.theta()-previousScore.theta())/previousScore.theta() < scoreStabilityThreshold &&
            Math.abs(currentScore.standardError()-previousScore.standardError())/previousScore.standardError() < errorStabilityThreshold) {
