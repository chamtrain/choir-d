...
public class OhsuCustomizer extends RegistryCustomizerDefault {

  private final Logger logger = LoggerFactory.getLogger(OhsuCustomizer.class);
  ...
  public OhsuCustomizer(SiteInfo siteInfo) {
    super(siteInfo);
  }
  ...
  public void scheduleSurveys(Database database, Date endDate, Long siteId) {
    ...
    OhsuSurveyScheduler scheduler = new OhsuSurveyScheduler(database, siteInfo);
  ...
  public ScoresExportReport getScoresExportReport(Database database, Map<String,String[]> params, SiteInfo siteInfo) {
   ...
   return new edu.stanford.registry.server.shc.ohsu

.OhsuSurveyData(database, siteInfo);

  }

  @Override
  public PatientReport getPatientReport(Database database, SiteInfo siteInfo) {
    return new OhsuPatientReport(database, siteInfo, ServerUtils.getAdminUser(database));
  }
  ...
  @Override
  public void batchSendEmails(DatabaseProvider dbp, AdministrativeServices adminServices) {
    try {
      super.batchSendEmails(dbp, adminServices); //runs once daily between 7am and 8am
      logger.info("Job batchSendEmails completed");
    } catch (Exception e) {
      logger.error("Error running batchSendEmails job for siteId:{} ", siteInfo.getIdString(), e);
    }
  }
