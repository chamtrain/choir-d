...
public class PainIntenErrorRenderer extends org.jfree.chart.renderer.xy.XYErrorRenderer {
  /**
   * Draws the error bars from the answers to "Least" and "Worst" pain.
   */

  private static final long serialVersionUID = 7801979753719741834L;
  private static Logger logger = Logger.getLogger(PainIntenErrorRenderer.class);
  ArrayList<LocalScore> stats;
  private boolean myDrawXError;
  private boolean myDrawYError;

  public PainIntenErrorRenderer(ArrayList<LocalScore> stats, boolean linesVisible, boolean shapesVisible) {
    super();
  ...
  /**
   * We're overriding drawItem to use the answers to question 1 as upper bound and question 3 as lower bound of the error bars.
   */
  @Override
  public void drawItem(Graphics2D g2, XYItemRendererState state, Rectangle2D dataArea, PlotRenderingInfo info,
    ...
    if (pass == 0) {
      ...
      // ArrayList<BigDecimal> answers = stats.get(item).getAnswers();
      Double worstAnswer = stats.get(item).getAnswer(RegistryShortFormScoreProvider.INTENSITY_QUESTION_WORST)
          .doubleValue();
      Double leastAnswer = stats.get(item).getAnswer(RegistryShortFormScoreProvider.INTENSITY_QUESTION_LEAST)
          .doubleValue();
      if (myDrawXError && item < stats.size()) {
        ...
        xx0 = xx0 -
            // answers.get(RegistryShortFormScoreProvider.INTENSITY_QUESTION_LEAST).doubleValue(); // == answer to 'least pain'
            leastAnswer; // pain'
        double xx1 = plot.getDomainAxis().valueToJava2D(x1, dataArea, edge);
        xx1 = xx1 +
            // answers.get(RegistryShortFormScoreProvider.INTENSITY_QUESTION_WORST).doubleValue(); // == answer 'worst pain'
            worstAnswer;
        ...
        Line2D cap2 = null;
        if (printDetails) {
          logger.debug("errorbars will be drawn from " + xx0 + " to " + xx1 + ", from least to worst ");
        }
      ...
      if (myDrawYError && item < stats.size()) {
        ...
        if (getItemShapeVisible(series, item)) {
          try {
            ...
            double errBarFrom = leastAnswer; // answers.get(RegistryShortFormScoreProvider.INTENSITY_QUESTION_LEAST).doubleValue();
            double errBarTo = worstAnswer; // answers.get(RegistryShortFormScoreProvider.INTENSITY_QUESTION_WORST).doubleValue();
            ...
            if (printDetails) {
              logger.debug("series " + series + " item " + item + " score=" + score + "error bar from least ="
                  + errBarFrom + " to worst =" + errBarTo + " point (x,y)=(" + pointX + "," + pointY + ") height:"
                  ...
                  + Math.round(yy0) + " to (y)=" + Math.round(yy1) + " XValue=" + dataset.getXValue(series, item)
            ...
            logger.error("Unable to create error bars for painIntensity " + ex.toString());
          }
        }
      }
    }
