 ...
 * until it needs more.
 */
public class RandomSetTenSet extends RandomSetter {
  private final static String ALGORITHM_NAME = "TSort";
  private final static Logger logger = LoggerFactory.getLogger(RandomSetTenSet.class);
  private final static RandomSetDao.Supplier myRsetDaoSupplier = new RandomSetDao.Supplier();
  private final static PatientDao.Supplier myPatDaoSupplier = new PatientDao.Supplier();

  private static Supplier<AddPatientAccessor> accessorSupplier;

  private final static int[] sizeSet = { 4, 6, 8, 10};
   ...
   */
  public RandomSetTenSet(Long siteId, String name) {
  ...
  private int getNextAssignmentAndUpdateDb(Database db, String stratumName) {
    ...
    // Plus there's a tiny chance a new SiteInfo will create a new RandomSetTSort for this RandomSet and use it
  ...
  static class AddPatientAccessor extends RandomSetDao.RandomSetStateAccessor {
    ...
    protected ArrayList<RandomSetGroup> createRandomList(RandomSetGroup plusThis) {
      ArrayList<RandomSetGroup> list = new ArrayList<>(rset.getBlockSize()+1);
      if (plusThis != null) {
        list.add(plusThis);
      }

      // Add each group to the list half the number of times selected from the size set
      int num = rand.nextInt(sizeSet.length);
