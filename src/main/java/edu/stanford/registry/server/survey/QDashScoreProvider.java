...
/**
 * Scoring for the Q-Dash survey. Created by tpacht on 07/29/2019.
 */
public class QDashScoreProvider extends ExtensibleScoreProvider {
  ...
  public ArrayList<ChartScore> getScore(PatientStudyExtendedData patientData) {
    ...
    try {
      QDashScore qDashScore = new QDashScore(patientData.getDtChanged(), patientData.getPatientId(),
      ...
      for (String columnName : columns.keySet()) {
        ...
        if (step != null){
          Integer score;
          if ("1".equals(parts[0]) || "4".equals(parts[0])) { // radiosetgrid questions
            score = surveyAdvanceUtils.getRadiosetgridChoice(step, fieldId);
          } else { // select1 questions
            score = surveyAdvanceUtils.selectedFieldChoice(step, fieldId);
          }
          if (score != null) {
            qDashScore.setAnswer(inx, new BigDecimal(score));
          }
        }
        inx++;
      }
      // Calculate score
      if (qDashScore.getAnswers().size() < 10) {
        // No score if more than 1 answer is missing
        logger.trace("No qDash score. Must have at least 10 responses. This one has only {} responses", qDashScore.getAnswers().size());
        qDashScore.setScore(new BigDecimal(-1));
      } else {
        BigDecimal responseSum = qDashScore.getScore();
        BigDecimal calculatedScore = new BigDecimal(((responseSum.doubleValue() / 11.0) - 1.0) * 25.0);
        BigDecimal roundedScore  = calculatedScore.setScale(2, BigDecimal.ROUND_HALF_EVEN);
        logger.trace("Sum of qDash responses is {} with a calculated score of {}", responseSum, calculatedScore);
        qDashScore.setScore(roundedScore);
        qDashScore.setAssisted(patientData.wasAssisted());
      }
      scores.add(qDashScore);
    } catch(Exception e){
  ...
  public XYDataset getTimeSet(TimeSeries baseLineSeries, ArrayList<ChartScore> stats, PrintStudy study,
    ...
    for (ChartScore stat1 : stats) {
      QDashScore score = (QDashScore) stat1;
  ...
  class QDashScore extends LocalScore {
    QDashScore(Date dt, String patientId, Integer studyCode, String description) {
    ...
    public Map<String, BigDecimal> getScores() {
      ...
      if (getScore() != null && getScore().intValue() > -1) {
        scores.put("QDASH_SCORE", getScore());
      }
      return scores;
    }
  }
