 ...
 */
public class SurveyAdvanceHandlerCOPCS extends SurveyAdvanceHandlerGenXmlSquareTable {
  private static final Logger log = Logger.getLogger(SurveyAdvanceHandlerCOPCS.class);


  private SurveySystem surveySystem = null;

  public SurveyAdvanceHandlerCOPCS(long configId, SiteInfo siteInfo) {
  ...
  public int getSurveySystemId(Database db) {
    if (surveySystem == null) {
      log.debug("Getting surveySystemId for COPCSService");
      surveySystem = new SurveySystDao(db).getSurveySystem("COPCSService");
    }
