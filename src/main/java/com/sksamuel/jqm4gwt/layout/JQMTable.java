 ...
 *
 *     Tables must have at least 1 and at most 8 columns
 *
...
public class JQMTable extends JQMWidget {
  ...
  public String getPercentageColumns() {
    ...
    for (int i = 0; i < percentage.length - 1; i++) {
      s.append(String.valueOf(percentage[i])).append(",");
    }
    s.append(String.valueOf(percentage[percentage.length - 1]));
    return s.toString();
  }
  ...
  public void showPercentageColumns(Integer... colIndexes) {
    ...
    Set<Integer> hideCols = new HashSet<>(percentage.length);
    for (int i = 0; i < percentage.length; i++) hideCols.add(i);
    for (Integer colIndexe : colIndexes) hideCols.remove(colIndexe);
  ...
  public Widget getCellWidget(int i) {
    return ((JQMTableCell) flow.getWidget(i)).getWidget();
  }

  public void rebaseCellWidget(int k) {
    Widget widget = flow.getWidget(k);
    String cellStyleName = getCellStyleName(k);
    removeAllCellStyles(widget.getElement());
    prepareCellPercentStyle(k, widget);
  ...
  public void replaceCellWidget(int i, Widget w) {
    //String cellStyleName = getCellStyleName(i);
    JQMTableCell cell =  ((JQMTableCell)flow.getWidget(i));
    cell.remove(0);
    cell.add(w);
    JQMContext.render(cell.getElement().getId());
    //w.getElement().addClassName(cellStyleName);
}
