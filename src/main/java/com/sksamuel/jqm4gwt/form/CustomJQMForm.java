...
/**
 * This is slightly modified version of JQMForm to fix a couple bugs and
 ...
 */
public class CustomJQMForm extends JQMForm {
    ...
    protected CustomJQMForm(SubmissionHandler<?> handler) {
        this();
    ...
    /**
     * Update the visibility of a widget contained within this form, taking into account
     * the validation labels (they will be shown/hidden as appropriate).
     *
     * @param widget the widget to show/hide
     * @param visible true to show, false to hide
     */
    public void setVisible(JQMFormWidget widget, boolean visible) {
        UIObject ui = widget.asWidget();
        ui.setVisible(visible);
        // Hide any validation labels associated with this widget
        Collection<Validator> validators = widgetValidators.get(widget);
        if (validators != null) {
            for (Validator v : validators) {
                Label label = validatorLabels.get(v);
                label.setVisible(visible && label.getText() != null && label.getText().trim().length() > 0);
            }
        }
    }
